...
class ChatbotEngine:
    ...
    def load_knowledge(self, path: str) -> Dict:
        ...
        if self.knowledge.semantic_enabled:
            logger.info(
                f"✅ Semantic search enabled with {self.knowledge.question_vectors.shape[0]} vectors"
            )
        else:
    ...
    def process_message(self, message: str, session_id: str = "default") -> Dict:
        ...
        logger.info(
            f"[{session_id}] intent={intent} conf={conf:.2f} "
    ...
    def _route(self, intent, entities, message, session_id, conf) -> Dict:
        ...
        if product_name or intent in ("ask_product", "buy_intent"):
            return self._handle_product(
                product_name or message, entities, session_id, message
            )
    ...
    def _handle_product(
        self, name_query: str, entities: Dict, session_id: str, raw_message: str
    ) -> Dict:
        """
    ...
    def _handle_search(self, intent, entities, message, session_id) -> Dict:
        ...
        if self.db and self.db.test_connection() and filters:
            products = self.db.search_products(
                {**filters, "limit": Config.MAX_PRODUCTS_RETURN}
            )
    ...
    def _build_filters(self, intent, entities, message) -> Dict:
        ...
        if entities.get("price_min"):
            f["min_price"] = entities["price_min"]
        if entities.get("price_max"):
            f["max_price"] = entities["price_max"]
        if entities.get("gender"):
            f["gender"] = entities["gender"]
        if entities.get("material"):
            f["frame_material"] = entities["material"]
        if entities.get("face_shape"):
            f["keyword"] = entities["face_shape"]
        if entities.get("style"):
            f["keyword"] = entities["style"]
    ...
    def _search_knowledge(self, query: str) -> str:
        ...
        if res:
            logger.info(
                f"  Knowledge hit: conf={res[0]['confidence']:.2f} "
    ...
    def _semantic_search_knowledge(self, query: str, threshold: float = 0.35) -> str:
        ...
        results = self.knowledge.search(
            query, top_k=1, threshold=threshold, use_hybrid=True
        )
    ...
    def _handle_negative(self, session_id, message, entities) -> Dict:
        """
        Xử lý phản hồi tiêu cực từ khách hàng
        """
    ...
    def _service_default(intent: str) -> str:
        ...
        return d.get(
            intent, "Bạn có thể hỏi chi tiết hơn không ạ? Shop sẵn sàng tư vấn!"
        )
