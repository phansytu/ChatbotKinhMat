...
class IntentClassifier:
    ...
    def classify(self, message: str) -> Dict:
        ...
        if entities.get("product_name") and primary not in (
            "feedback_negative",
            "greeting",
            "thanks",
            "goodbye",
    ...
    def _extract_product_name(self, raw: str, norm: str) -> Optional[str]:
        ...
        for brand in KNOWN_BRANDS:
            if brand in raw_lower:
                ...
                fragment = re.sub(
                    r"^(kính|gọng|mắt kính|cho|của|hỏi|về)\s*",
                    "",
                    fragment,
                    flags=re.IGNORECASE,
                )
                fragment = re.sub(
                    r"\s*(giá|bao nhiêu|có không|như thế nào|\?).*$",
                    "",
                    fragment,
                    flags=re.IGNORECASE,
                ).strip()
    ...
    def _extract_price(self, msg: str) -> Dict:
        ...
        patterns = [
            ...
            (
                r"(?:dưới|không quá|tối đa|đến)\s*(\d[\d,\.]*)\s*(?:k|nghìn|ngàn|triệu|tr\b)",
                "max",
            ),
            (
                r"(?:tầm|khoảng|cỡ)\s*(\d[\d,\.]*)\s*(?:k|nghìn|ngàn|triệu|tr\b)",
                "approx",
            ),
            (
                r"(\d[\d,\.]*)\s*(?:k|nghìn|ngàn|triệu|tr\b)\s*[-–đến]+\s*(\d[\d,\.]*)\s*(?:k|nghìn|ngàn|triệu|tr\b)",
                "range",
            ),
        ]
    ...
    def _extract_gender(msg: str) -> Optional[str]:
        if any(
            w in msg
            for w in ["cho nam", "của nam", " nam ", "con trai", "bạn trai", "đàn ông"]
        ):
            return "male"
        if any(
            w in msg
            for w in ["cho nữ", "của nữ", " nữ ", "con gái", "bạn gái", "phụ nữ"]
        ):
            return "female"
        if "unisex" in msg:
            return "unisex"
    ...
    def detect_intent_semantic(
        self, message: str, semantic_results: list = None
    ) -> Dict:
        """
        ...
        if semantic_results and len(semantic_results) > 0:
            ...
            if "có những loại" in matched_question or "phân loại" in matched_question:
                ...
                primary = "ask_compare"
                conf = max(conf, semantic_conf)
            elif "mặt" in matched_question and (
                "tròn" in matched_question or "vuông" in matched_question
            ):
    ...
    def debug_intent(self, message: str) -> Dict:
        ...
        entities = self._extract_entities(
            message, norm, matched[0][0] if matched else "unknown"
        )
        ...
        print(
            f"Primary intent: {matched[0][0] if matched else 'unknown'} (conf={matched[0][1] if matched else 0:.2f})"
        )
