...
class ProductAdvisor:
    ...
    def build_product_reply(
        ...
        if disc and disc > 0:
            parts.append(
                f"💰 **Giá:** {self._fmt(price)} ~~{self._fmt(orig)}~~ (-{disc:.0f}% giảm)"
            )
        else:
        ...
        if rating and reviews > 0:
            stars = "⭐" * int(rating)
            parts.append(
                f"{stars} Đánh giá: **{rating}/5** ({reviews} đánh giá, {sold} đã bán)"
            )
    ...
    def build_multi_product_reply(
        self, products: List[Dict], knowledge_snippet: str = ""
    ) -> str:
        """
    ...
    def build_type_query_reply(self, category: str, knowledge_content: str) -> str:
        ...
        # Chọn template intro
        intro = self._pick(
            self.TYPE_QUERY_INTRO, category=category, content=knowledge_content
        )
    ...
    def build_type_query_with_products(
        self, category: str, knowledge_content: str, products: List[Dict]
    ) -> str:
        """
    ...
    def extract_type_info(self, knowledge_content: str, max_length: int = 2000) -> Dict:
        ...
        result = {"main_types": [], "sub_types": {}, "recommendations": []}
        ...
        for line in lines:
            ...
            # Phát hiện gợi ý (chứa "gợi ý", "nên chọn", "phù hợp")
                ...
                for keyword in ["gợi ý", "nên chọn", "phù hợp", "theo nhu cầu"]
            ):
    ...
    def build_contextual_reply(
        self, intent: str, entities: Dict, products: List[Dict], knowledge: str
    ) -> str:
        """
    ...
    def build_face_advice(
        self, face_shape: str, knowledge: str, products: List[Dict]
    ) -> str:
    ...
    def build_price_recommendation(
        self, entities: Dict, products: List[Dict], knowledge: str
    ) -> str:
        """Tư vấn theo giá"""
